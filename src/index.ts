...
export default function vitePluginImplicitCssModules(): Plugin {
  return {
    ...
    transform(src: string, id: string) {
      ...
      // 匹配获取所有的 <style> 内容
      const reg =
        /<style(?:\slang="\w+")?\s*module(=["']\w+["'])?>[^<]+<\/style>/gim;
      ...
      styleCodes.forEach((styleCode: RegExpMatchArray) => {
        ...
        const templateReg = /<template>[\s\S]*<\/template>/gim;
        const templateCode = src.match(templateReg);
        // 处理 template 中的 class
        if (templateCode) {
          const resolved = resolveCssModules(
            styleCode[0],
            templateCode[0],
            moduleId
          );
...
function resolveCssModules(
  ...
  // 移除 :global 声明
  styleStr = styleStr.replace(/:global[^{]*/gim, '');
  ...
  multiClassNames.forEach((str: string) => {
    ...
    const moduleClassNameArr = classNameArr.filter((item: string) =>
    ...
    if (moduleClassNameArr.length) {
      ...
      moduleClassStr += moduleClassNameArr
        .map((item: string) => `${moduleId}['${item}']`)
        .join(',');
      moduleClassStr += ']"';
    }

    // 普通 class name
    classNameArr = classNameArr.filter(
      (item: string) => !(classNames || []).includes(item) && item
    );
    // 处理普通 class
    let classStr = '';
